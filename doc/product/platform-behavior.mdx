...
| OHOS (OpenHarmony / HarmonyOS) | Restarts the whole process and relaunches the UIAbility through `ApplicationContext.restartApp(want)` (API 12+).                                                                                                                                                                  | Requires the app to be in the foreground; the call must run on the main thread, and repeated calls within 3 seconds may fail. Flutter engine restart is not available.                                       |
