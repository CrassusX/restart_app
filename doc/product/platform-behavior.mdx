...
| OHOS (OpenHarmony / HarmonyOS) | Restarts the whole process and relaunches the UIAbility through `ApplicationContext.restartApp(want)` (API 12+). On older API levels it falls back to `startAbility(want)` + `terminateSelf()`.                                                   | Requires the app to be focused when restarting. Flutter engine restart is not available.                                                                                                                     |
