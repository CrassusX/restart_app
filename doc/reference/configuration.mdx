...
On OHOS (OpenHarmony / HarmonyOS), `RestartMode.platformDefault` and `RestartMode.process` both restart the whole process through `ApplicationContext.restartApp(want)` (API 12+), falling back to `startAbility` + `terminateSelf` on older API levels. `forceKill` is implied because the process is always restarted. `flutterEngine` and `notificationFallback` are unsupported and return a failed result.
