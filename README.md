...
| **OHOS**    | Restarts the whole process and relaunches the UIAbility through `ApplicationContext.restartApp(want)` (API 12+).                                                                                                                                 | Requires the app to be in the foreground; the call must run on the main thread, and repeated calls within 3 seconds may fail. Flutter engine restart is not available.                                       |
...
On OHOS, `restart_app` restarts the app using the native `ApplicationContext.restartApp(want)` API, which relaunches the current UIAbility and gives a clean cold start.

- `RestartMode.platformDefault` and `RestartMode.process` both use `ApplicationContext.restartApp(want)` (API 12+).
- The app must be in the foreground and the call must run on the main thread.
- Successful calls should not be repeated within 3 seconds, or the platform may return error `16000064`.
